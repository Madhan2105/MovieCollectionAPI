...
class CollectionTest(APITestCase):
    ...
    def test_create_collection(self):
        ...
        self.assertEqual(str(Collection.objects.get().id),str(response.data["collection_uuid"]))
    ...
    def test_update_collection(self):        
        """
            Ensure we are ale to fetch collection on basis of collection id.
        """                
        data = {
            "title": "2023 fav movies",
            "description":"This are the movies i loved in the year 2023",
            "movies":[
                {
                    "title": "Queerama",
                    "description": "53 years after decriminalisation of homosexuality in the UK, director Daisy Asquith mines the jewels of the BFI archive to take us into the relationships, desires, fears and expressions of gay men and women in the 20th century.",
                    "genres": "comedy",
        ...
        collection_id = str(collection.id)
        url = reverse('collection')
        request = self.factory.put(url+"/"+collection_id,data=data, format="json")             
        force_authenticate(request, user=self.user)
        response = self.view(request,collection_id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Collection.objects.filter(is_active=1).count(), 1)        
